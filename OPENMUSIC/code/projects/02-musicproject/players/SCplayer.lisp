...
(defun open-SCplayer ()
  (unless *SC-listen-process*
    (setf *SC-listen-process* (om-start-osc-server *SCplayer-in-port* *SCplayer-host*  #'send-more-notes))))
...
;; (om-send-osc-bundle *SCplayer-out-port* *SCplayer-host*  '(("/play.sc_om" 1209 1210 12 13)))
...
(defun send-200 ()
  (loop for i from 1 to 200
        while (< *index-packets* (length *SCosc-packets*)) do
