...
(defmethod! decompo ((n integer))
  ...
  :doc "Etant donne un n correspondent à la periode d'un pattern rythmique  qui genere  par des translations temporelles, 
un canon regulier de categorie maximale, cette fonction le decompose dans un produit de cinq nombres (p1, p2, n1, n2, n3).
attaches à un canon ayant comme nombre des voix le produit de n1 et n2 et 
comme nombre d'attaques du pattern rythmique de base le produit des trois nombres restants." 
   :icon 420 
...
(defmethod! Infocanons ((n integer))
     :initvals '(72) :indoc '("periode")
   :icon 420
   "Donne une liste ayant comme arguments le nombre de voix du canon et le nombre d'attaques du pattern rythmique de base à l'interieur d'un periode."
   (if (good-n-p n)
