...
(defparameter *fluid-sf2* (merge-pathnames (make-pathname :directory '(:relative "resources/in-files") :name "merlin.sf2") *om-root*))
...
(defmethod get-def-vals ((ID (eql :fluid)))
    (list 
          :fluid-sf2 (merge-pathnames (make-pathname :directory '(:relative "resources/in-files") :name "merlin.sf2") *om-root*)
          :n-fsynth 1
          :fluid-autoload nil 
  ...
  (let ((thescroll (om-make-view 'preference-pane
    ...
    (when (cl-fluid::fluidsynthlib-p)
      (om-add-subviews thescroll 
                       ...
                       (om-make-dialog-item 'om-check-box (om-make-point 210 (- i 5)) (om-make-point 20 20) ""
                                            :font *controls-font*
                                            :checked-p (get-pref modulepref :fluid-auto-microtune)
                                            :di-action (om-dialog-item-act item 
                                                         (set-pref modulepref :fluid-auto-microtune (om-checked-p item))
                                                         ))
