...
(defmethod set-sf2-directory-view ((self sf2-ports-view) dialog) 
  (let* ((devices (remove nil (loop for ref in (cl-fluid::name-fsynths cl-fluid::*fl-synths*) ;(list-devices) 
        ...
        (oa::om-with-delayed-update self
          ...
          (when settings-list
            (apply 'oa::om-add-subviews 
                   (cons self
                         (setf (portlines self)
                               (loop for portsetting in cl-fluid::*fl-synths* ;(sort settings-list '< :key 'second)
                                         ...
                                         (oa::om-add-subviews vv
                                                              ...
                                                              (om-make-view 'om-icon-button 
                                                                            ...
                                                                            :action (om-dialog-item-act item
                                                                                      (declare (ignore item))
                                                                                      (let ((newsf2 (om-choose-file-dialog :directory (pathname *fluid-sf2*)
                                                              ...
                                                              (setf sf2txt (om-make-dialog-item 'om-static-text  (om-make-point 120 i) (om-make-point 320 45)
                                                                                                (om-namestring *fluid-sf2* ;(get-pref modulepref :fluid-sf2)
                                                                                                               )
