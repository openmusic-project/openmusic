...
(defmethod om-draw-contents ((self soundPanel))
  ...
  (if (om-sound-file-name (object (om-view-container self)))
    (let* ((thesound (object (om-view-container self)))
           (sr oa::srate)
           (dur (or (and (and sr (oa::number-of-samples-current thesound))
                         (/ (oa::number-of-samples-current thesound) sr)) 0))
           (dur-ms (round (oa::number-of-samples-current thesound) (/ sr 1000.0)))
