...
(defmethod Preparetoplay ((player t) (s sound-data) at &key approx port interval voice)
  (when (ptr s)
    (when *audio-player*
      (om-add-sound-to-player *audio-player* (ptr s) at (first interval) (second interval) (tracknum s))
      )))
...
(defmethod! Sound-Play ((s sound-data))
  :icon 207
  (when (ptr s)
    (om-play-one-sound (ptr s) *audio-player*))
  )
...
(setf *faust-folder* *om-infiles-folder*)
...
(defmethod! faust-effect ((s sound-data) effect &optional values)
  :icon 106
  (let ((efflist (las::MakeAudioEffectList))
        (fausteffect (las::MakeFaustAudioEffect (namestring effect))))
    (if (las::las-null-ptr-p fausteffect)
      (om-beep-msg "Bad effect file!")
      (let ((nparams (las::getcontrolcount fausteffect)))
        (print "FAUST Effect")
        (print effect)
        (print (format nil "~d Params" nparams))
        (loop for i from 0 to (- nparams 1) 
              for v in values do
              (print (multiple-value-list (las::getcontrolparam fausteffect i)))
              (las::SetControlValue fausteffect i (float v))
              (print (format nil "Set to ~d" (las::GetControlValue fausteffect i)))
              )
        (setq efflist (las::AddAudioEffect efflist fausteffect))
        (make-instance 'sound-data 
          :ptr (las::MakeTransformSound (ptr s) efflist 100 100)
          :tracknum (tracknum s))
        ))
    ))

(defmethod! faust-effect ((s1 sound) effect &optional values)
  :icon 231
  (faust-effect (get-sound-data s1) effect values))




;(setf efflist (las::MakeAudioEffectList))
;(setf fx (sdif-null-ptr-p (las::MakeFaustAudioEffect (om-path2cmdpath "HD:Users:jean:Desktop:moduledir:freeverb.so")))
;(om-path2cmdpath "/Users/jean/Desktop/moduledir/freeverb.so")
;(las::getcontrolcount fx) 