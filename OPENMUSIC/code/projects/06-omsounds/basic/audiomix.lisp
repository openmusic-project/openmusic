...
(defvar  *audiorealtime* "si les modifs sont envoyees au player en temps reel")
(setf *audiorealtime* t)
...
;  (when *audiorealtime* (om-set-track-volume *audio-player* (track (channelctr self)) (float (/ value 100))))
...
(defmethod change-volume ((self audiochannelPanel) value)
  (setf (vol-ctrl (channelctr self)) value)
  (change-channel-vol *audio-player-visible* (- (track (channelctr self)) 1) (float (/ value 100)))
  (let ((new-str (integer-to-string value))
        (target (volumeVal self)))
    (unless (string= new-str (om-dialog-item-text target))
      (om-set-dialog-item-text target new-str)
...
;  (when *audiorealtime* (om-set-track-pan *audio-player* (track (channelctr self)) (- 1.0 (float (/ (+ value 100) 200)))))
...
(defmethod change-pan ((self audiochannelPanel) value)
  (setf (pan-ctrl (channelctr self)) value)
  (change-channel-pan *audio-player-visible* (- (track (channelctr self)) 1) (- 1.0 (float (/ (+ value 100) 200))))
  (let* ((target (panVal self))
         (new-str (audio-pan2str value)))
    (unless (string= new-str (om-dialog-item-text target))
      (om-set-dialog-item-text target new-str)
...
(defmethod send-audio-settings ((self audio-mix-console))
  (when *audio-player*
    (loop for chan-ctrl in (channels-ctrl self) do
        (send-audio-settings chan-ctrl))))

(defmethod send-audio-settings ((self audio-channel-ctrl))
  (om-set-track-volume *audio-player* (track self) (float (/ (vol-ctrl self) 100)))
  (om-set-track-pan *audio-player* (track self) (- 1.0 (float (/ (+ 100 (pan-ctrl self)) 200))))
  )

(defmethod handle-key-event ((self audioControllerPanel) char)
  (case char
    (#\space 
     ;(play (object (view-container self)) :port (midiport (object (view-container self))))
     (send-audio-settings (object (om-view-container self)))
     )
    (otherwise (om-beep))))


(defmethod* PrepareToPlay ((player t) (self audio-mix-console) at &key approx port interval voice)
  (declare (ignore approx))
  (call-next-method)
  t)

(defmethod DoPlay ((self audio-mix-console) start end)
  (call-next-method)
  (send-audio-settings self)
  )

(defmethod DoStop ((self audio-mix-console)) t)

(defmethod DoPause ((self audio-mix-console)) t)

(defmethod DoContinue ((self audio-mix-console)) t)
