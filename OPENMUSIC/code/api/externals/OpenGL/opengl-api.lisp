      ...
      (case character
        ...
        (#\n (if (nice-texture-p viewer)
             ...
             (when (texturep (icosahedron viewer))
               ...
               (turn-on-texture viewer))
             t)
        #-(or lispworks7 lispworks8) ((#\begin #\home) 
		      (opengl:rendering-on ((canvas viewer))
			(initialize-transform (icotransform viewer))
			(initialize-transform (light-transform viewer)) t)
		      (setf (xyz-y (eye (camera (canvas viewer))))
			    (xyz-y *eye*)))
        #-(or lispworks7 lispworks8) (#\Insert (opengl:rendering-on ((canvas viewer)) (reset-lights-and-materials) t))
        #-(or lispworks7 lispworks8) (#\escape (opengl:rendering-on ((canvas viewer)) (delete-display-list (icosahedron viewer)))
