...
(defun gen-reference (entries dir &key title maintext)
  ...
  (let ((title (or title (concatenate 'string "OM " *version-string*)))
    ...
    (with-open-file (index indexpath :direction :output :external-format :utf-8)
    ...
    (with-open-file (index alphaindexpath :direction :output :external-format :utf-8)
      (write-line "<html>" index)
...
(defun make-ref-page (symbol dir &optional title)
  (let ((title (or title ""))
    ...
    (with-open-file (index pagepath :direction :output :if-exists :supersede :external-format :utf-8)
      (write-line "<html>" index)
