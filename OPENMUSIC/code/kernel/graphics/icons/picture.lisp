...
(defmethod draw-pict-patch ((self patch-picture) view)
   (let ((pictsize (pict-size self))
     ...
     (if (thepict self) 
         (om-draw-picture view (thepict self) (om-make-point x0 y0) pictsize)
  ...
  (let* ((pictsize (om-get-picture-size pict))
    ...
    (loop for i from 0 to sizex do
          (loop for j from 0 to sizey do
                (om-draw-picture view pict (om-make-point (* i x) (* j y)) 
                                 (om-make-point x y))))))
      ...
      (cond ((equal 'c mode)
             (let* ((syse (get-system-etat view))
               ...
               (om-draw-picture view (thepict self)
                                ...
                                (om-subtract-points (point2pixel view (om-make-point x2 y2) syse)
                                                    (point2pixel view (om-make-point x y) syse)))))
        ((equal 'p mode)
         (let ((syse (get-system-etat view))
               (x 0) (y (om-pict-height (thepict self)))
               (x2 (om-pict-width (thepict self))) (y2 0))
           (om-draw-picture view (thepict self) (point2pixel view (om-make-point x y) syse)
                            (om-subtract-points (point2pixel view (om-make-point x2 y2) syse)
                                                (point2pixel view (om-make-point x y) syse)))))
        ((equal 'w mode)
         (om-draw-picture view (thepict self) (om-make-point 0 0) (om-view-size view)))
        ((equal 'm mode)
